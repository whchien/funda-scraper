...
class FundaScraper(object):
    ...
    async def fetch_one_parent(self, session, parent_url):
        async with session.get(parent_url, headers=config.header) as response:
    ...
    async def fetch_all_links(self, session) -> None:
        ...
        tasks = []
        for i in range(0, self.n_pages + 1):
            parent_url = f"{main_url}&search_result={i}"
            task = asyncio.create_task(self.fetch_one_parent(session=session, parent_url=parent_url))
            tasks.append(task)
    ...
    async def main(self):
        async with aiohttp.ClientSession() as session:
            result = await self.fetch_all_links(session=session)
            print(result)
